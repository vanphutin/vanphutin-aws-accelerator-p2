# Bài 18: Sử dụng Volume: emptyDir và hostPath

## Mục tiêu

Sau bài học này, bạn cần nắm được chủ đề chính: Volume gắn vào Pod để chia sẻ dữ liệu; emptyDir sống theo Pod; hostPath phụ thuộc node và cần dùng rất cẩn trọng. Bài thuộc nhóm kiến thức **State Persistence** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Chủ đề của bài này nằm trong domain **State Persistence**. Trong bài gốc của series DevOps Việt Nam, phần này được đặt ở thứ tự bài 18 và tập trung vào nội dung: Volume gắn vào Pod để chia sẻ dữ liệu; emptyDir sống theo Pod; hostPath phụ thuộc node và cần dùng rất cẩn trọng. Tài liệu dưới đây viết lại bằng lời mới, mở rộng thêm các điểm thực tế thường gặp trong cluster hiện đại như `EndpointSlice`, `autoscaling/v2`, `startupProbe`, `metrics-server`, `StorageClass` hoặc workflow `--dry-run=client -o yaml` tùy theo bài.
...
Volume gắn vào Pod để chia sẻ dữ liệu; emptyDir sống theo Pod; hostPath phụ thuộc node và cần dùng rất cẩn trọng.
...
```mermaid
graph TD
  Pod["Pod"] --> Empty["emptyDir"]
  Pod --> Host["hostPath"]
  C1["Container A"] --> Empty
  C2["Container B"] --> Empty
  Host --> NodeFS["Node filesystem"]
```
...
### `kubectl exec volume-demo -c writer -- ls /data`
Kiểm tra file trong mount emptyDir.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `exec`: Subcommand xác định hành động cần thực hiện.
* `volume-demo`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `-c`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `writer`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `ls`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `/data`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl logs volume-demo -c reader`
Xem container reader đọc dữ liệu từ volume chung.
...
* `volume-demo`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `-c`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `reader`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl describe pod volume-demo`
Xem danh sách volumes và mounts.
...
Xóa Pod để chứng minh emptyDir mất theo Pod.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `delete`: Subcommand xác định hành động cần thực hiện.
* `pod`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `volume-demo`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-18.yaml`:
...
spec:
  ...
  - name: writer
    image: busybox:1.36
    command: ["sh", "-c", "while true; do date > /cache/time.txt; sleep 5; done"]
    volumeMounts:
    - name: cache
      mountPath: /cache
  - name: reader
    image: busybox:1.36
    command: ["sh", "-c", "while true; do cat /cache/time.txt 2>/dev/null || true; sleep 5; done"]
    volumeMounts:
    - name: cache
      mountPath: /cache
```
...
```bash
kubectl apply -f lesson-18.yaml
```
...
Tạo Pod writer/reader dùng emptyDir, xem log reader; sau đó xóa tạo lại Pod để thấy dữ liệu không còn.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-18`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-18`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-18.yaml` và xóa namespace nếu không dùng nữa.
...
* Dùng emptyDir cho dữ liệu cần tồn tại sau khi Pod mất.
* Dùng hostPath trong workload portable.
* Mount đè lên thư mục có sẵn làm mất file image tại mount point.
...
Bài này giúp bạn nắm nội dung: Volume gắn vào Pod để chia sẻ dữ liệu; emptyDir sống theo Pod; hostPath phụ thuộc node và cần dùng rất cẩn trọng. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
