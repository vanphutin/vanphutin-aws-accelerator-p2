...
Sau bài học này, bạn cần nắm được chủ đề chính: Requests phục vụ scheduling; limits giới hạn runtime; QoS class ảnh hưởng eviction khi node thiếu tài nguyên. Bài thuộc nhóm kiến thức **Pod Design** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Requests phục vụ scheduling; limits giới hạn runtime; QoS class ảnh hưởng eviction khi node thiếu tài nguyên.
...
```mermaid
graph TD
  Pod["Pod resources"] --> Req["requests"]
  Pod --> Lim["limits"]
  Req --> Scheduler["scheduler chon node"]
  Lim --> Kubelet["kubelet/cgroup enforcement"]
  Req --> QoS["QoS class"]
  Lim --> QoS
```
...
Xem request/limit và QoS class.
...
* `resource-demo`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl top pod resource-demo --containers`
Xem CPU/memory sử dụng thực tế nếu metrics-server hoạt động.
...
* `jsonpath='{.status.qosClass}'`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl describe node <node-name>`
Kiểm tra allocatable và tài nguyên đã được request trên node.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-12.yaml`:
...
```bash
kubectl apply -f lesson-12.yaml
```
...
Tạo Pod có request/limit, xem QoS, sau đó thử bỏ limit hoặc request để quan sát QoS thay đổi.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-12`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-12`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-12.yaml` và xóa namespace nếu không dùng nữa.
...
* Đặt limit CPU quá thấp làm tăng latency.
* Không đặt request khiến scheduling thiếu dự đoán.
* Nhầm Mi với M hoặc m CPU với core.
...
Bài này giúp bạn nắm nội dung: Requests phục vụ scheduling; limits giới hạn runtime; QoS class ảnh hưởng eviction khi node thiếu tài nguyên. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
