...
Sau bài học này, bạn cần nắm được chủ đề chính: Tạo Pod bằng lệnh và YAML, đọc phase, condition, restart count, event và xóa tài nguyên đúng cách. Bài thuộc nhóm kiến thức **Core Concepts** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Tạo Pod bằng lệnh và YAML, đọc phase, condition, restart count, event và xóa tài nguyên đúng cách.
...
Đọc chi tiết container, volume, event và nguyên nhân lỗi.
...
Sinh manifest mẫu để chỉnh sửa declarative.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-03.yaml`:
...
```bash
kubectl apply -f lesson-03.yaml
```
...
Sinh YAML bằng --dry-run, apply file, cố tình đổi image sai, quan sát ImagePullBackOff bằng describe, rồi sửa lại image hợp lệ.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-03`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-03`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-03.yaml` và xóa namespace nếu không dùng nữa.
...
* Chỉ nhìn STATUS mà không đọc Events.
* Xóa Pod do Deployment quản lý rồi ngạc nhiên vì Pod được tạo lại.
* Không đặt label nên về sau khó expose Service.
...
Bài này giúp bạn nắm nội dung: Tạo Pod bằng lệnh và YAML, đọc phase, condition, restart count, event và xóa tài nguyên đúng cách. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
