# Bài 16: Sử dụng Ingress và NetworkPolicy

## Mục tiêu

Sau bài học này, bạn cần nắm được chủ đề chính: Ingress định tuyến HTTP/HTTPS layer 7; NetworkPolicy kiểm soát traffic vào/ra Pod khi CNI hỗ trợ. Bài thuộc nhóm kiến thức **Services & Networking** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Ingress định tuyến HTTP/HTTPS layer 7; NetworkPolicy kiểm soát traffic vào/ra Pod khi CNI hỗ trợ.
...
```mermaid
graph TD
  Internet["Client"] --> Ingress["Ingress"]
  Ingress --> Svc["Service"]
  Svc --> Pod["Pods"]
  Policy["NetworkPolicy"] -. allow/deny .-> Pod
```
...
Bật ingress-nginx trên Minikube.
...
* `ingress`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl get ingress`
Xem rule host/path và address.
...
Debug rule, backend và event của Ingress.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `describe`: Subcommand xác định hành động cần thực hiện.
* `ingress`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `web-ingress`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `apply`: Subcommand xác định hành động cần thực hiện.
* `-f`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `networkpolicy.yaml`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl get networkpolicy`
Liệt kê policy trong namespace.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `get`: Subcommand xác định hành động cần thực hiện.
* `networkpolicy`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-16.yaml`:
...
```bash
kubectl apply -f lesson-16.yaml
```
...
Tạo Service web trước, apply Ingress, map host web.local vào IP Minikube, sau đó áp NetworkPolicy và test Pod có/không có label role=client.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-16`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-16`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-16.yaml` và xóa namespace nếu không dùng nữa.
...
* Quên Ingress controller.
* Dùng apiVersion cũ extensions/v1beta1.
* Áp NetworkPolicy nhưng CNI local không enforce.
...
Bài này giúp bạn nắm nội dung: Ingress định tuyến HTTP/HTTPS layer 7; NetworkPolicy kiểm soát traffic vào/ra Pod khi CNI hỗ trợ. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
