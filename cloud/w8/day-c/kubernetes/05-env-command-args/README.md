...
Sau bài học này, bạn cần nắm được chủ đề chính: Inject biến môi trường, dùng downward API, override ENTRYPOINT/CMD bằng command và args. Bài thuộc nhóm kiến thức **Configuration** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Inject biến môi trường, dùng downward API, override ENTRYPOINT/CMD bằng command và args.
...
```mermaid
graph TD
  Spec["Pod spec"] --> Env["env/envFrom"]
  Spec --> Cmd["command"]
  Spec --> Args["args"]
  Env --> Process["Container process"]
  Cmd --> Process
  Args --> Process
```
...
### `kubectl run env-pod --image=busybox:1.36 --env=APP_MODE=dev --command -- sleep 3600`
Tạo Pod có env tĩnh và command tùy chỉnh.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `run`: Subcommand xác định hành động cần thực hiện.
* `env-pod`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--image=busybox:1.36`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--env=APP_MODE=dev`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--command`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `sleep`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl exec env-pod -- env`
Chạy env bên trong container để kiểm tra biến.
...
* `env-pod`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `env`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl explain pod.spec.containers.env`
Tra cứu schema env ngay trong terminal.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `explain`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `pod.spec.containers.env`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl logs env-command-pod`
Xem output do command/args ghi ra stdout.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `logs`: Subcommand xác định hành động cần thực hiện.
* `env-command-pod`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-05.yaml`:
...
image: busybox:1.36
command: ["sh"]
args: ["-c", "echo pod=$POD_NAME ns=$POD_NAMESPACE mode=$APP_MODE; sleep 3600"]
env:
- name: APP_MODE
  value: training
- name: POD_NAME
...
```bash
kubectl apply -f lesson-05.yaml
```
...
Tạo Pod, kiểm tra log, exec vào Pod để chạy env, sau đó đổi APP_MODE trong YAML và apply lại bằng delete/apply vì nhiều field của Pod là immutable.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-05`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-05`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-05.yaml` và xóa namespace nếu không dùng nữa.
...
* Nhầm command của Kubernetes với CMD của Docker.
* Viết command dạng chuỗi dài thay vì mảng.
* Không quote giá trị số hoặc boolean trong env.
...
Bài này giúp bạn nắm nội dung: Inject biến môi trường, dùng downward API, override ENTRYPOINT/CMD bằng command và args. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
