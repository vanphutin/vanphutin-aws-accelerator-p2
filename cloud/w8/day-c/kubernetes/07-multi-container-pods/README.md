...
Sau bài học này, bạn cần nắm được chủ đề chính: Hiểu khi nào nhiều container nên cùng ở một Pod: cùng lifecycle, network namespace và volume. Bài thuộc nhóm kiến thức **Multi-Container Pods** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Hiểu khi nào nhiều container nên cùng ở một Pod: cùng lifecycle, network namespace và volume.
...
Xem log container app trong Pod nhiều container.
...
* `multi-pod`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `-c`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `helper`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl exec -it multi-pod -c app -- sh`
Exec vào container app cụ thể.
...
Kiểm tra trạng thái từng container trong cùng Pod.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-07.yaml`:
...
```bash
kubectl apply -f lesson-07.yaml
```
...
Tạo Pod hai container, xem log helper đang đọc file app ghi ra volume chung, rồi xóa Pod để thấy emptyDir biến mất theo Pod.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-07`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-07`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-07.yaml` và xóa namespace nếu không dùng nữa.
...
* Gộp các service độc lập vào cùng Pod.
* Quên chỉ định -c khi logs/exec.
* Dùng Pod nhiều container để thay thế Service mesh hoặc queue.
...
Bài này giúp bạn nắm nội dung: Hiểu khi nào nhiều container nên cùng ở một Pod: cùng lifecycle, network namespace và volume. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
