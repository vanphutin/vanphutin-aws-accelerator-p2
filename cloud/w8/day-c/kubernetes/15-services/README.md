# Bài 15: Cấu hình Service: ClusterIP và NodePort

## Mục tiêu

Sau bài học này, bạn cần nắm được chủ đề chính: Service chọn Pod bằng selector, tạo virtual IP ổn định và chuyển traffic tới EndpointSlice. Bài thuộc nhóm kiến thức **Services & Networking** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Service chọn Pod bằng selector, tạo virtual IP ổn định và chuyển traffic tới EndpointSlice.
...
* `expose`: Subcommand xác định hành động cần thực hiện.
...
* `--port=80`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--target-port=80`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--type=ClusterIP`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl get svc web`
Xem ClusterIP, port và type của Service.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `get`: Subcommand xác định hành động cần thực hiện.
* `svc`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `web`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl get endpoints web`
Xem endpoint backend tương ứng selector.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `get`: Subcommand xác định hành động cần thực hiện.
* `endpoints`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `web`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl port-forward service/web 8080:80`
Forward Service về máy local để test.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `port-forward`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `service/web`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `8080:80`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl expose deployment web --port=80 --type=NodePort --name=web-nodeport`
Tạo Service NodePort cho truy cập qua node.
...
* `web`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--port=80`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--type=NodePort`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--name=web-nodeport`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-15.yaml`:
...
```bash
kubectl apply -f lesson-15.yaml
```
...
Apply Deployment/Service, kiểm tra endpoints, chạy Pod busybox để wget http://web, rồi đổi selector sai để thấy endpoints rỗng.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-15`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-15`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-15.yaml` và xóa namespace nếu không dùng nữa.
...
* Selector Service không khớp label Pod.
* Nhầm port với targetPort.
* Dùng NodePort cho mọi thứ thay vì Ingress/LoadBalancer phù hợp.
...
Bài này giúp bạn nắm nội dung: Service chọn Pod bằng selector, tạo virtual IP ổn định và chuyển traffic tới EndpointSlice. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
