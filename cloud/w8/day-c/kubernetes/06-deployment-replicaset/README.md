# Bài 6: Cấu hình Deployment và ReplicaSet

## Mục tiêu

Sau bài học này, bạn cần nắm được chủ đề chính: Quản lý ứng dụng stateless bằng Deployment, ReplicaSet, rollout, rollback và selector bất biến. Bài thuộc nhóm kiến thức **Configuration** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Chủ đề của bài này nằm trong domain **Configuration**. Trong bài gốc của series DevOps Việt Nam, phần này được đặt ở thứ tự bài 6 và tập trung vào nội dung: Quản lý ứng dụng stateless bằng Deployment, ReplicaSet, rollout, rollback và selector bất biến. Tài liệu dưới đây viết lại bằng lời mới, mở rộng thêm các điểm thực tế thường gặp trong cluster hiện đại như `EndpointSlice`, `autoscaling/v2`, `startupProbe`, `metrics-server`, `StorageClass` hoặc workflow `--dry-run=client -o yaml` tùy theo bài.
...
Quản lý ứng dụng stateless bằng Deployment, ReplicaSet, rollout, rollback và selector bất biến.
...
```mermaid
graph TD
  Deploy["Deployment"] --> RS1["ReplicaSet revision 1"]
  Deploy --> RS2["ReplicaSet revision 2"]
  RS2 --> P1["Pod"]
  RS2 --> P2["Pod"]
  RS2 --> P3["Pod"]
```
...
Tạo Deployment nhanh với ba replica.
...
* `--image=nginx:1.27-alpine`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--replicas=3`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl rollout status deployment/web`
Theo dõi rollout cho tới khi hoàn tất hoặc lỗi.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `rollout`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `status`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `deployment/web`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `set`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `image`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `deployment/web`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `nginx=nginx:1.26-alpine`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `rollout`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `undo`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `deployment/web`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl scale deployment web --replicas=5`
Thay đổi số replica mong muốn.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `scale`: Subcommand xác định hành động cần thực hiện.
* `deployment`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `web`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--replicas=5`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-06.yaml`:
...
```bash
kubectl apply -f lesson-06.yaml
```
...
Apply Deployment, cập nhật image, xem ReplicaSet cũ/mới, rollback và xác nhận Pod template quay về revision trước.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-06`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-06`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-06.yaml` và xóa namespace nếu không dùng nữa.
...
* Selector không khớp label template.
* Sửa selector sau khi tạo Deployment.
* Quên kiểm tra rollout status sau khi đổi image.
...
Bài này giúp bạn nắm nội dung: Quản lý ứng dụng stateless bằng Deployment, ReplicaSet, rollout, rollback và selector bất biến. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
