...
Sau bài học này, bạn cần nắm được chủ đề chính: Thiết lập cluster local bằng Minikube, cấu hình kubectl, context, namespace và thao tác CRUD cơ bản. Bài thuộc nhóm kiến thức **Core Concepts** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Thiết lập cluster local bằng Minikube, cấu hình kubectl, context, namespace và thao tác CRUD cơ bản.
...
```mermaid
graph LR
  Host["May local"] --> Driver["Docker/Hyper-V/VirtualBox driver"]
  Driver --> MK["Minikube node"]
...
Khởi tạo cluster local dùng Docker driver.

Giải thích thành phần:
* `minikube`: CLI quản lý cluster local Minikube.
* `start`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--driver=docker`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl config current-context`
Xác nhận kubectl đang trỏ tới context đúng.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `config`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `current-context`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl get namespaces`
Liệt kê namespace hiện có trong cluster.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `get`: Subcommand xác định hành động cần thực hiện.
* `namespaces`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl create namespace ckad-lab`
Tạo namespace riêng cho bài thực hành.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `create`: Subcommand xác định hành động cần thực hiện.
* `namespace`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `ckad-lab`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `config`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `set-context`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--current`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--namespace=ckad-lab`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-02.yaml`:
...
```bash
kubectl apply -f lesson-02.yaml
```
...
Cài hoặc khởi động Minikube, tạo namespace ckad-lab, chuyển context sang namespace này và kiểm tra bằng kubectl config view --minify.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-02`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-02`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-02.yaml` và xóa namespace nếu không dùng nữa.
...
* Quên kiểm tra context trước khi apply YAML.
* Lẫn namespace default với namespace lab.
* Dùng lệnh imperative nhưng không lưu YAML để tái lập.
...
Bài này giúp bạn nắm nội dung: Thiết lập cluster local bằng Minikube, cấu hình kubectl, context, namespace và thao tác CRUD cơ bản. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
