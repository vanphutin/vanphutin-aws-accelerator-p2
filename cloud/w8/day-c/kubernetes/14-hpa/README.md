# Bài 14: Tối ưu hóa Pod với HPA

## Mục tiêu

Sau bài học này, bạn cần nắm được chủ đề chính: Horizontal Pod Autoscaler scale Deployment theo metrics, thường là CPU utilization, cần metrics-server và resource requests. Bài thuộc nhóm kiến thức **Pod Design** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Horizontal Pod Autoscaler scale Deployment theo metrics, thường là CPU utilization, cần metrics-server và resource requests.
...
* `minikube`: CLI quản lý cluster local Minikube.
* `addons`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `enable`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `metrics-server`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl autoscale deployment php-apache --cpu-percent=50 --min=1 --max=5`
Tạo HPA cho Deployment dựa trên CPU.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `autoscale`: Subcommand xác định hành động cần thực hiện.
* `deployment`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `php-apache`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--cpu-percent=50`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--min=1`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--max=5`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl get hpa`
Xem target, current metric và replica hiện tại.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `get`: Subcommand xác định hành động cần thực hiện.
* `hpa`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl describe hpa php-apache`
Đọc event và điều kiện scaling.
...
### `kubectl top pods`
Kiểm tra metrics thực tế của Pod.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-14.yaml`:
...
```bash
kubectl apply -f lesson-14.yaml
```
...
Bật metrics-server, apply Deployment/HPA, tạo load bằng busybox wget loop, quan sát replicas tăng rồi giảm khi ngừng tải.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-14`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-14`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-14.yaml` và xóa namespace nếu không dùng nữa.
...
* Quên CPU request nên HPA không tính utilization.
* Kỳ vọng scale tức thì.
* Không kiểm tra metrics-server khi current metric unknown.
...
Bài này giúp bạn nắm nội dung: Horizontal Pod Autoscaler scale Deployment theo metrics, thường là CPU utilization, cần metrics-server và resource requests. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
