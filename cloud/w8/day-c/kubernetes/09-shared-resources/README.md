...
Sau bài học này, bạn cần nắm được chủ đề chính: Chia sẻ network namespace, localhost, volume và tài nguyên CPU/memory giữa container trong Pod. Bài thuộc nhóm kiến thức **Multi-Container Pods** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Chia sẻ network namespace, localhost, volume và tài nguyên CPU/memory giữa container trong Pod.
...
```mermaid
graph TD
  Pod["Pod sandbox"] --> Net["shared network namespace"]
  Pod --> Vol["shared emptyDir"]
  Net --> C1["container A"]
  Net --> C2["container B"]
  Vol --> C1
  Vol --> C2
...
### `kubectl exec shared-demo -c client -- wget -qO- http://127.0.0.1:8080`
Từ container client gọi service local của container server.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `exec`: Subcommand xác định hành động cần thực hiện.
* `shared-demo`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `-c`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `client`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `wget`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `-qO-`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl top pod shared-demo --containers`
Xem tài nguyên từng container nếu metrics-server có sẵn.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `top`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `pod`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `shared-demo`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--containers`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl describe pod shared-demo`
Kiểm tra mount, port và request/limit từng container.
...
### `kubectl logs shared-demo -c server`
...
* `shared-demo`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `-c`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `server`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-09.yaml`:
...
spec:
  ...
  - name: writer
    image: busybox:1.36
    command: ["sh", "-c", "while true; do echo hello-from-writer > /html/index.html; sleep 10; done"]
    volumeMounts:
    - name: html
      mountPath: /html
```
...
```bash
kubectl apply -f lesson-09.yaml
```
...
Tạo Pod server/writer, port-forward hoặc exec từ container khác để đọc nội dung index.html được writer cập nhật.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-09`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-09`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-09.yaml` và xóa namespace nếu không dùng nữa.
...
* Nghĩ container trong Pod có IP riêng.
* Quên mỗi container vẫn có filesystem riêng nếu không mount volume.
* Không đặt resources cho từng container.
...
Bài này giúp bạn nắm nội dung: Chia sẻ network namespace, localhost, volume và tài nguyên CPU/memory giữa container trong Pod. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
