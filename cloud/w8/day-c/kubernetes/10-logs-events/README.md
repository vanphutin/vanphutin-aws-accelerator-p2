...
Sau bài học này, bạn cần nắm được chủ đề chính: Truy xuất stdout/stderr, log container trước khi restart, event theo thời gian và workflow debug CrashLoopBackOff. Bài thuộc nhóm kiến thức **Observability** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Truy xuất stdout/stderr, log container trước khi restart, event theo thời gian và workflow debug CrashLoopBackOff.
...
```mermaid
graph TD
  Container["Container stdout/stderr"] --> Runtime["container runtime log"]
  Runtime --> Kubelet["kubelet"]
  Kubelet --> Kubectl["kubectl logs"]
  API["API server"] --> Events["Events"]
  Events --> GetEvents["kubectl get events"]
```
...
Lấy log container mặc định của Pod.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `logs`: Subcommand xác định hành động cần thực hiện.
* `log-pod`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl logs log-pod -f --tail=20`
Follow log realtime và chỉ lấy 20 dòng gần nhất ban đầu.
...
* `--tail=20`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl logs crash-pod --previous`
Lấy log của container instance trước đó sau khi restart.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `logs`: Subcommand xác định hành động cần thực hiện.
* `crash-pod`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--previous`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl get events --sort-by=.metadata.creationTimestamp`
Sắp xếp event theo thời gian tạo.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `get`: Subcommand xác định hành động cần thực hiện.
* `events`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--sort-by=.metadata.creationTimestamp`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl logs multi-pod -c app`
Lấy log container cụ thể trong Pod nhiều container.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `logs`: Subcommand xác định hành động cần thực hiện.
* `multi-pod`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `-c`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `app`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-10.yaml`:
...
spec:
  ...
  - name: logger
image: busybox:1.36
command: ["sh", "-c", "i=0; while true; do echo log-$i; i=$((i+1)); sleep 5; done"]
```

Áp dụng manifest:

```bash
kubectl apply -f lesson-10.yaml
```
...
Tạo Pod ghi log liên tục, dùng --tail, --since và -f; sau đó tạo Pod exit 1 để luyện --previous.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-10`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-10`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-10.yaml` và xóa namespace nếu không dùng nữa.
...
* Tìm log trong file bên trong container thay vì stdout/stderr.
* Quên --previous khi container đã restart.
* Không xem Events khi log không đủ thông tin.
...
Bài này giúp bạn nắm nội dung: Truy xuất stdout/stderr, log container trước khi restart, event theo thời gian và workflow debug CrashLoopBackOff. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
