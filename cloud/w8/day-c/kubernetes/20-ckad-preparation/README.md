# Bài 20: Tổng kết, mẹo thi CKAD và bài tập thực hành cuối

## Mục tiêu

Sau bài học này, bạn cần nắm được chủ đề chính: Ôn toàn bộ domain CKAD, chiến thuật dùng kubectl nhanh, quản lý thời gian và bài lab tổng hợp. Bài thuộc nhóm kiến thức **CKAD Preparation** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Chủ đề của bài này nằm trong domain **CKAD Preparation**. Trong bài gốc của series DevOps Việt Nam, phần này được đặt ở thứ tự bài 20 và tập trung vào nội dung: Ôn toàn bộ domain CKAD, chiến thuật dùng kubectl nhanh, quản lý thời gian và bài lab tổng hợp. Tài liệu dưới đây viết lại bằng lời mới, mở rộng thêm các điểm thực tế thường gặp trong cluster hiện đại như `EndpointSlice`, `autoscaling/v2`, `startupProbe`, `metrics-server`, `StorageClass` hoặc workflow `--dry-run=client -o yaml` tùy theo bài.
...
Ôn toàn bộ domain CKAD, chiến thuật dùng kubectl nhanh, quản lý thời gian và bài lab tổng hợp.
...
Tra cứu schema nhanh khi không nhớ field.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `explain`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `pod.spec.containers`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--recursive`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `|`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `less`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl create deployment web --image=nginx --dry-run=client -o yaml > deploy.yaml`
Sinh YAML nhanh từ imperative command.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `create`: Subcommand xác định hành động cần thực hiện.
* `deployment`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `web`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--image=nginx`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `--dry-run=client`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
...
Tạo Pod tạm để test DNS/network.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `run`: Subcommand xác định hành động cần thực hiện.
* `tmp`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
Kiểm tra quyền RBAC trong context hiện tại.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `auth`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `can-i`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `create`: Subcommand xác định hành động cần thực hiện.
* `deployments`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl get all,cm,secret,pvc`
Tổng kiểm tài nguyên thường gặp trong namespace.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `get`: Subcommand xác định hành động cần thực hiện.
* `all,cm,secret,pvc`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-20.yaml`:
...
```bash
kubectl apply -f lesson-20.yaml
```
...
Trong 25 phút, tạo Deployment có resource/probe, Service, ConfigMap, Secret, Pod test DNS và ghi lại các lệnh verify.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-20`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-20`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-20.yaml` và xóa namespace nếu không dùng nữa.
...
* Không đọc kỹ namespace trong đề.
* Dành quá lâu cho một câu.
* Không verify sau khi apply.
* Không dùng --dry-run để tạo YAML nhanh.
...
Bài này giúp bạn nắm nội dung: Ôn toàn bộ domain CKAD, chiến thuật dùng kubectl nhanh, quản lý thời gian và bài lab tổng hợp. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
