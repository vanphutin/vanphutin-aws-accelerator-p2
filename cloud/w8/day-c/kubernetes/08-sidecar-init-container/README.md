...
Sau bài học này, bạn cần nắm được chủ đề chính: Init container chạy trước container chính; sidecar chạy cùng để bổ trợ logging, proxy, sync hoặc config rendering. Bài thuộc nhóm kiến thức **Multi-Container Pods** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Chủ đề của bài này nằm trong domain **Multi-Container Pods**. Trong bài gốc của series DevOps Việt Nam, phần này được đặt ở thứ tự bài 8 và tập trung vào nội dung: Init container chạy trước container chính; sidecar chạy cùng để bổ trợ logging, proxy, sync hoặc config rendering. Tài liệu dưới đây viết lại bằng lời mới, mở rộng thêm các điểm thực tế thường gặp trong cluster hiện đại như `EndpointSlice`, `autoscaling/v2`, `startupProbe`, `metrics-server`, `StorageClass` hoặc workflow `--dry-run=client -o yaml` tùy theo bài.
...
Init container chạy trước container chính; sidecar chạy cùng để bổ trợ logging, proxy, sync hoặc config rendering.
...
### `kubectl get pod sidecar-demo -o jsonpath='{.status.initContainerStatuses[*].state}'`
Xem trạng thái init container.
...
* `sidecar-demo`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `-o`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `jsonpath='{.status.initContainerStatuses[*].state}'`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl logs sidecar-demo -c init-config`
Xem log init container sau khi đã chạy.
...
* `init-config`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl logs sidecar-demo -c sidecar -f`
Theo dõi sidecar đang chạy song song.
...
* `sidecar-demo`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `-c`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `sidecar`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `-f`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl describe pod sidecar-demo`
Đọc thứ tự start, restart count và event.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-08.yaml`:
...
spec:
  ...
  - name: app
    image: busybox:1.36
    command: ["sh", "-c", "while true; do cat /work/status.txt; sleep 10; done"]
    volumeMounts:
    - name: workdir
      mountPath: /work
  - name: sidecar
    image: busybox:1.36
    command: ["sh", "-c", "while true; do date >> /work/sidecar.log; sleep 5; done"]
    volumeMounts:
    - name: workdir
      mountPath: /work
```
...
```bash
kubectl apply -f lesson-08.yaml
```
...
Tạo Pod, kiểm tra init container đã hoàn tất, xem app đọc file do init tạo và sidecar ghi log định kỳ.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-08`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-08`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-08.yaml` và xóa namespace nếu không dùng nữa.
...
* Đặt tác vụ chạy mãi trong init container.
* Dùng sidecar cho việc chỉ cần chạy một lần.
* Quên volume chung giữa init/app/sidecar.
...
Bài này giúp bạn nắm nội dung: Init container chạy trước container chính; sidecar chạy cùng để bổ trợ logging, proxy, sync hoặc config rendering. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
