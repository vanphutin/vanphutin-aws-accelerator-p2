...
Sau bài học này, bạn cần nắm được chủ đề chính: Tách cấu hình khỏi image, dùng ConfigMap cho dữ liệu không nhạy cảm và Secret cho dữ liệu nhạy cảm. Bài thuộc nhóm kiến thức **Configuration** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
Tách cấu hình khỏi image, dùng ConfigMap cho dữ liệu không nhạy cảm và Secret cho dữ liệu nhạy cảm.
...
Tạo ConfigMap từ một cặp key/value.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `create`: Subcommand xác định hành động cần thực hiện.
* `configmap`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `app-config`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--from-literal=APP_MODE=prod`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl create secret generic app-secret --from-literal=API_KEY=change-me`
Tạo Secret generic từ literal.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `create`: Subcommand xác định hành động cần thực hiện.
* `secret`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `generic`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `app-secret`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `--from-literal=API_KEY=change-me`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.

### `kubectl get configmap app-config -o yaml`
Xem cấu trúc YAML của ConfigMap.
...
Lấy giá trị đã base64 encode của một key trong Secret.
...
* `app-secret`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `-o`: Flag điều chỉnh phạm vi, output hoặc hành vi của lệnh.
* `jsonpath='{.data.API_KEY}'`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-04.yaml`:
...
spec:
  ...
  - name: app
image: busybox:1.36
command: ["sh", "-c", "echo mode=$APP_MODE level=$LOG_LEVEL key=$API_KEY; sleep 3600"]
envFrom:
- configMapRef:
    name: app-config
- secretRef:
...
```bash
kubectl apply -f lesson-04.yaml
```
...
Apply manifest, xem log để xác nhận biến môi trường đã được inject, sau đó xóa Secret và tạo lại để hiểu vòng đời tham chiếu.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-04`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-04`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-04.yaml` và xóa namespace nếu không dùng nữa.
...
* Nghĩ Secret tự động được mã hóa mạnh chỉ vì base64.
* Quên optional khiến Pod không khởi động khi thiếu key.
...
Bài này giúp bạn nắm nội dung: Tách cấu hình khỏi image, dùng ConfigMap cho dữ liệu không nhạy cảm và Secret cho dữ liệu nhạy cảm. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
