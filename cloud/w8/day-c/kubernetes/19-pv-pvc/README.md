# Bài 19: Cấu hình PersistentVolume và PersistentVolumeClaim

## Mục tiêu

Sau bài học này, bạn cần nắm được chủ đề chính: PV đại diện storage trong cluster; PVC là yêu cầu storage của workload; StorageClass hỗ trợ dynamic provisioning. Bài thuộc nhóm kiến thức **State Persistence** trong lộ trình CKAD. Mục tiêu không chỉ là nhớ khái niệm mà còn thao tác được bằng `kubectl`, đọc được trạng thái tài nguyên, viết được YAML đầy đủ và biết tự kiểm chứng kết quả.
...
PV đại diện storage trong cluster; PVC là yêu cầu storage của workload; StorageClass hỗ trợ dynamic provisioning.
...
Xem StorageClass mặc định nếu có.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `get`: Subcommand xác định hành động cần thực hiện.
* `storageclass`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl get pv,pvc`
Liệt kê PV và PVC cùng phase Bound/Pending.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `get`: Subcommand xác định hành động cần thực hiện.
* `pv,pvc`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl describe pvc app-data`
Debug lý do PVC Pending.

Giải thích thành phần:
* `kubectl`: CLI gửi request tới Kubernetes API server.
* `describe`: Subcommand xác định hành động cần thực hiện.
* `pvc`: Đối tượng, tên tài nguyên, image hoặc tham số của command.
* `app-data`: Đối tượng, tên tài nguyên, image hoặc tham số của command.

### `kubectl delete pvc app-data`
Xóa claim; reclaimPolicy của PV quyết định dữ liệu phía sau.
...
Lưu manifest sau vào file phù hợp, ví dụ `lesson-19.yaml`:
...
spec:
  ...
  - name: app
image: busybox:1.36
command: ["sh", "-c", "echo persistent > /data/message.txt; sleep 3600"]
volumeMounts:
- name: data
  mountPath: /data
  volumes:
...
```bash
kubectl apply -f lesson-19.yaml
```
...
Trên Minikube, apply PVC/Pod dùng StorageClass mặc định, xóa Pod rồi tạo Pod khác mount cùng PVC để đọc file cũ.

Các bước gợi ý:

1. Tạo namespace riêng: `kubectl create namespace lesson-19`.
2. Chuyển namespace: `kubectl config set-context --current --namespace=lesson-19`.
...
6. Dọn dẹp bằng `kubectl delete -f lesson-19.yaml` và xóa namespace nếu không dùng nữa.
...
* Quên cluster không có StorageClass mặc định.
* Nhầm accessModes với quyền Linux file.
* Xóa PVC khi chưa hiểu reclaimPolicy.
...
Bài này giúp bạn nắm nội dung: PV đại diện storage trong cluster; PVC là yêu cầu storage của workload; StorageClass hỗ trợ dynamic provisioning. Bạn đã có manifest chạy được, bộ command cần nhớ, quy trình debug và các lỗi hay gặp. Hãy luyện cho tới khi bạn có thể tự tạo YAML từ trí nhớ ở mức khung chính, sau đó dùng `kubectl explain` để tra field chi tiết. Khi đọc tài liệu Kubernetes, hãy ưu tiên phiên bản API hiện hành và tránh ví dụ đã lỗi thời.
