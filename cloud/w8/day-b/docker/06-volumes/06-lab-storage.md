...
- Hiểu bản chất của `inspect`, `permissions`, `cleanup`.
...
Trong Docker, chủ đề **Lab storage** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Lab storage"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Lab storage** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker volume inspect` | Thành phần `docker volume inspect` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `app-data` | Thành phần `app-data` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
