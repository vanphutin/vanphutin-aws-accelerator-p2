...
- Hiểu bản chất của `named volume`, `driver`, `persistence`.
...
Trong Docker, chủ đề **Docker Volume** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Docker Volume"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Docker Volume** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker run -d -v app-data:/data alpine sleep 3600
```
...
| `-v` | Thành phần `-v` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `app-data:/data` | Thành phần `app-data:/data` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `alpine` | Thành phần `alpine` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `sleep 3600` | Thành phần `sleep 3600` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
