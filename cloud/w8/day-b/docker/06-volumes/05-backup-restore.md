...
- Hiểu bản chất của `backup`, `restore`, `tar`, `consistency`.
...
Trong Docker, chủ đề **Backup và Restore volume** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Backup và Restore volume"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Backup và Restore volume** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker run --rm -v app-data:/data -v ${PWD}:/backup alpine tar czf /backup/app-data.tgz -C /data .
```
...
| `-v app-data:/data` | Thành phần `-v app-data:/data` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `-v ${PWD}:/backup` | Thành phần `-v ${PWD}:/backup` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `alpine` | Thành phần `alpine` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `tar czf /backup/app-data.tgz -C /data .` | Thành phần `tar czf /backup/app-data.tgz -C /data .` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
