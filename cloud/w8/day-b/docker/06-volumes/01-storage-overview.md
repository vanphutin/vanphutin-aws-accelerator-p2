...
- Hiểu bản chất của `writable layer`, `volume`, `bind mount`, `tmpfs`.
...
Trong Docker, chủ đề **Tổng quan storage** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Tổng quan storage"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Tổng quan storage** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker volume ls` | Thành phần `docker volume ls` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
