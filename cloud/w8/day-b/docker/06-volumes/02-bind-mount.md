...
- Hiểu bản chất của `host path`, `read-only`, `development`.
...
Trong Docker, chủ đề **Bind Mount** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Bind Mount"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Bind Mount** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `${PWD}:/usr/share/nginx/html:ro` | Thành phần `${PWD}:/usr/share/nginx/html:ro` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
