# Lab nền tảng Kubernetes

Chapter: Kubernetes Foundation

## Mục tiêu

- Hiểu bản chất của `deploy`, `expose`, `logs`.
...
Trong Docker, chủ đề **Lab nền tảng Kubernetes** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Lab nền tảng Kubernetes"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Lab nền tảng Kubernetes** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `kubectl expose deployment` | Thành phần `kubectl expose deployment` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `web` | Thành phần `web` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--port=80` | Thành phần `--port=80` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--type=NodePort` | Thành phần `--type=NodePort` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
