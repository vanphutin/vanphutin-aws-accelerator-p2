...
- Hiểu bản chất của `pod`, `deployment`, `service`, `replicaset`.
...
Trong Docker, chủ đề **Pod, Deployment, Service** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Pod, Deployment, Service"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Pod, Deployment, Service** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `kubectl create deployment` | Thành phần `kubectl create deployment` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `web` | Thành phần `web` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--image=nginx` | Thành phần `--image=nginx` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
