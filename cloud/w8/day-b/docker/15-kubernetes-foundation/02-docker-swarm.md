...
- Hiểu bản chất của `manager`, `worker`, `service`, `replica`.
...
Trong Docker, chủ đề **Docker Swarm** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Docker Swarm"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Docker Swarm** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker service create` | Thành phần `docker service create` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
