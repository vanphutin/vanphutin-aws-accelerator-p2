# containerd, runc, CRI và OCI Runtime

Chapter: Kubernetes Foundation

## Mục tiêu

- Hiểu bản chất của `containerd`, `runc`, `CRI`, `OCI Runtime`.
...
Trong Docker, chủ đề **containerd, runc, CRI và OCI Runtime** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["containerd, runc, CRI và OCI Runtime"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **containerd, runc, CRI và OCI Runtime** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `ctr version` | Thành phần `ctr version` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
