...
- Hiểu bản chất của `cluster`, `node`, `control plane`, `pod`.
...
Trong Docker, chủ đề **Kubernetes Introduction** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Kubernetes Introduction"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Kubernetes Introduction** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `kubectl get nodes` | Thành phần `kubectl get nodes` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
