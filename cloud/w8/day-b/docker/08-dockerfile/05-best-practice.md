...
- Hiểu bản chất của `small base`, `non-root`, `pin version`, `healthcheck`.
...
Trong Docker, chủ đề **Dockerfile Best Practices** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Dockerfile Best Practices"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Dockerfile Best Practices** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker build --pull -t secure-app .
```
...
| `-t secure-app` | Thành phần `-t secure-app` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
