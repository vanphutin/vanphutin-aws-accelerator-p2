...
- Hiểu bản chất của `builder stage`, `runtime stage`, `artifact copy`.
...
Trong Docker, chủ đề **Multi-stage Build** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Multi-stage Build"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Multi-stage Build** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker build -t app-prod --target runtime .
```
...
| `-t app-prod` | Thành phần `-t app-prod` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--target runtime` | Chỉ build đến stage có tên `runtime` trong multi-stage Dockerfile. |
