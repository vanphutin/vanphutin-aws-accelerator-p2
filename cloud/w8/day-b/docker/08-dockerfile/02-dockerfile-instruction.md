...
- Hiểu bản chất của `FROM`, `RUN`, `CMD`, `ENTRYPOINT`, `COPY`, `ADD`, `ENV`, `ARG`, `WORKDIR`, `EXPOSE`, `USER`, `HEALTHCHECK`.
...
Trong Docker, chủ đề **Dockerfile Instructions** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Dockerfile Instructions"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Dockerfile Instructions** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker build --no-cache -t instruction-demo .
```
...
| `--no-cache` | Không dùng cache cũ khi build. |
| `-t instruction-demo` | Thành phần `-t instruction-demo` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
