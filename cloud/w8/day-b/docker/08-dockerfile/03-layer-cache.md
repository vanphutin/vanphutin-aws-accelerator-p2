...
- Hiểu bản chất của `cache key`, `layer reuse`, `invalidating cache`.
...
Trong Docker, chủ đề **Layer Cache** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Layer Cache"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Layer Cache** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
DOCKER_BUILDKIT=1 docker build --progress=plain -t cache-demo .
```
...
|---|---|
| `DOCKER_BUILDKIT=1` | Thành phần `DOCKER_BUILDKIT=1` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `docker build` | Build image từ Dockerfile và build context. |
| `--progress=plain` | Thành phần `--progress=plain` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `-t cache-demo` | Thành phần `-t cache-demo` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
