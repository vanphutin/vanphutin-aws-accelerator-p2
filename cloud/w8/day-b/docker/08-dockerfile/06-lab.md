...
- Hiểu bản chất của `build`, `run`, `test`, `optimize`.
...
Trong Docker, chủ đề **Lab Dockerfile** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Lab Dockerfile"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Lab Dockerfile** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `demo-app:1.0` | Thành phần `demo-app:1.0` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
