# Environment và dependency

Chapter: Docker Compose

## Mục tiêu

- Hiểu bản chất của `environment`, `env_file`, `depends_on`, `profiles`.
...
Trong Docker, chủ đề **Environment và dependency** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Environment và dependency"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Environment và dependency** trên máy local trước khi đưa vào pipeline hoặc server lab.
