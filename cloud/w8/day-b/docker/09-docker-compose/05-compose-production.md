# Compose cho production nhỏ

Chapter: Docker Compose

## Mục tiêu

- Hiểu bản chất của `restart policy`, `secrets`, `resource limits`.
...
Trong Docker, chủ đề **Compose cho production nhỏ** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Compose cho production nhỏ"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Compose cho production nhỏ** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `--scale worker=3` | Thành phần `--scale worker=3` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
