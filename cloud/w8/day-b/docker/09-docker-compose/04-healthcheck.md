...
- Hiểu bản chất của `healthcheck`, `condition`, `startup order`.
...
Trong Docker, chủ đề **Healthcheck trong Compose** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Healthcheck trong Compose"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Healthcheck trong Compose** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker compose ps` | Thành phần `docker compose ps` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
