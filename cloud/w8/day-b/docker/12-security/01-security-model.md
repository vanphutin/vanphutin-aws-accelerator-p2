...
- Hiểu bản chất của `attack surface`, `least privilege`, `read-only fs`.
...
Trong Docker, chủ đề **Mô hình bảo mật Docker** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Mô hình bảo mật Docker"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Mô hình bảo mật Docker** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `--read-only` | Thành phần `--read-only` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
