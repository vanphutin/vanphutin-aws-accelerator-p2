...
- Hiểu bản chất của `rootless`, `user namespace`, `uid mapping`.
...
Trong Docker, chủ đề **Rootless Docker và User Namespace** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Rootless Docker và User Namespace"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Rootless Docker và User Namespace** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker info` | Thành phần `docker info` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--format '{{.SecurityOptions}}'` | Thành phần `--format '{{.SecurityOptions}}'` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
