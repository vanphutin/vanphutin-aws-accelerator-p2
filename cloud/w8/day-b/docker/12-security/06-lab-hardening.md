# Lab hardening container

Chapter: Docker Security

## Mục tiêu

- Hiểu bản chất của `capabilities`, `no-new-privileges`, `non-root`.
...
Trong Docker, chủ đề **Lab hardening container** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Lab hardening container"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Lab hardening container** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker run --rm --cap-drop ALL --security-opt no-new-privileges nginx:alpine
```
...
| `--cap-drop ALL` | Thành phần `--cap-drop ALL` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--security-opt no-new-privileges` | Thành phần `--security-opt no-new-privileges` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
