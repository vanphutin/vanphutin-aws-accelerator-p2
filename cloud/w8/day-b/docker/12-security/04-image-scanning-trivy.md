...
- Hiểu bản chất của `CVE`, `severity`, `SBOM`, `policy`.
...
Trong Docker, chủ đề **Image Scanning với Trivy** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Image Scanning với Trivy"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Image Scanning với Trivy** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `trivy image` | Thành phần `trivy image` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
