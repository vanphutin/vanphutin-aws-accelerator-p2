...
- Hiểu bản chất của `CIS benchmark`, `host audit`, `hardening`.
...
Trong Docker, chủ đề **Docker Bench Security** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Docker Bench Security"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Docker Bench Security** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker run --rm --net host --pid host --userns host --cap-add audit_control docker/docker-bench-security
```
...
| `--net host` | Thành phần `--net host` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--pid host` | Thành phần `--pid host` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--userns host` | Thành phần `--userns host` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--cap-add audit_control` | Thành phần `--cap-add audit_control` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `docker/docker-bench-security` | Thành phần `docker/docker-bench-security` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
