...
- Hiểu bản chất của `run`, `start`, `stop`, `restart`.
...
Trong Docker, chủ đề **Run, Start, Stop, Restart** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Run, Start, Stop, Restart"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Run, Start, Stop, Restart** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker restart` | Thành phần `docker restart` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
