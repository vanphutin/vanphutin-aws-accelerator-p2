...
- Hiểu bản chất của `debug`, `shell`, `inspect`, `logs`.
...
Trong Docker, chủ đề **Logs, Exec, Inspect** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Logs, Exec, Inspect"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Logs, Exec, Inspect** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker exec` | Thành phần `docker exec` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
...
| `sh` | Thành phần `sh` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
