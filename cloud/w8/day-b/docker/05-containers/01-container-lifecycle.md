...
- Hiểu bản chất của `created`, `running`, `paused`, `exited`, `removed`.
...
Trong Docker, chủ đề **Vòng đời container** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Vòng đời container"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Vòng đời container** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `--name app` | Thành phần `--name app` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
