# Stats, Top và resource

Chapter: Docker Containers

## Mục tiêu

- Hiểu bản chất của `CPU`, `memory`, `process`, `limits`.
...
Trong Docker, chủ đề **Stats, Top và resource** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Stats, Top và resource"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Stats, Top và resource** trên máy local trước khi đưa vào pipeline hoặc server lab.
