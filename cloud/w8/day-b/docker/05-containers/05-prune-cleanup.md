...
- Hiểu bản chất của `stopped containers`, `cleanup`, `data risk`.
...
Trong Docker, chủ đề **Prune và cleanup** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Prune và cleanup"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Prune và cleanup** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker container prune` | Thành phần `docker container prune` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
