...
- Hiểu bản chất của `official image`, `rate limit`, `namespace`.
...
Trong Docker, chủ đề **Docker Hub** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Docker Hub"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Docker Hub** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker pull` | Tải image từ registry về máy local. |
| `library/nginx:alpine` | Thành phần `library/nginx:alpine` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
