...
- Hiểu bản chất của `local registry`, `push`, `pull`.
...
Trong Docker, chủ đề **Lab registry local** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Lab registry local"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Lab registry local** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `5000:5000` | Thành phần `5000:5000` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--name registry` | Thành phần `--name registry` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `registry:2` | Thành phần `registry:2` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
