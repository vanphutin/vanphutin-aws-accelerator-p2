# Project 8: Harbor Registry

Chapter: Real-world Projects

## Mục tiêu

- Hiểu bản chất của `harbor`, `registry`, `TLS`.
...
Trong Docker, chủ đề **Project 8: Harbor Registry** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Project 8: Harbor Registry"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Project 8: Harbor Registry** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `./install.sh` | Thành phần `./install.sh` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
...
Project có mã nguồn chạy được trong thư mục con `projects/08-harbor-registry` của chapter này. Hãy vào đúng thư mục project trước khi chạy command.
