# Project 4: React + Nginx

Chapter: Real-world Projects

## Mục tiêu

- Hiểu bản chất của `react`, `nginx`, `multi-stage`.
...
Trong Docker, chủ đề **Project 4: React + Nginx** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Project 4: React + Nginx"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Project 4: React + Nginx** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker build` | Build image từ Dockerfile và build context. |
| `-t react-nginx` | Thành phần `-t react-nginx` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
...
Project có mã nguồn chạy được trong thư mục con `projects/04-react-nginx` của chapter này. Hãy vào đúng thư mục project trước khi chạy command.
