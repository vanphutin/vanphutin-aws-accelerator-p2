# Project 10: Dockerized Microservices

Chapter: Real-world Projects

## Mục tiêu

- Hiểu bản chất của `api`, `worker`, `database`, `gateway`.
...
Trong Docker, chủ đề **Project 10: Dockerized Microservices** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Project 10: Dockerized Microservices"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Project 10: Dockerized Microservices** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker compose up` | Tạo và chạy toàn bộ service trong file Compose. |
| `-d` | Chạy ở chế độ nền, trả lại terminal ngay sau khi container/service khởi động. |
| `--build` | Build lại image trước khi chạy service. |
...
Project có mã nguồn chạy được trong thư mục con `projects/10-dockerized-microservices` của chapter này. Hãy vào đúng thư mục project trước khi chạy command.
