...
- Hiểu bản chất của `github actions`, `scan`, `push`.
...
Trong Docker, chủ đề **Project 9: CI/CD Pipeline** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Project 9: CI/CD Pipeline"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Project 9: CI/CD Pipeline** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker buildx build` | Thành phần `docker buildx build` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--push` | Thành phần `--push` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `-t ghcr.io/org/app:latest` | Thành phần `-t ghcr.io/org/app:latest` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
...
Project có mã nguồn chạy được trong thư mục con `projects/09-ci-cd-pipeline` của chapter này. Hãy vào đúng thư mục project trước khi chạy command.

## Quy trình chạy nhanh

1. Đọc file `README.md` trong thư mục project.
2. Chạy command chính của project.
3. Kiểm tra container bằng `docker compose ps`.
4. Xem log bằng `docker compose logs -f`.
5. Dọn dẹp bằng `docker compose down -v` nếu muốn xóa cả volume lab.
