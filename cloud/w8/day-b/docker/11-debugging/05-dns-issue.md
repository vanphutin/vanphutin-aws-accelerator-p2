...
- Hiểu bản chất của `DNS config`, `network`, `resolver`.
...
Trong Docker, chủ đề **DNS Issue** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["DNS Issue"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **DNS Issue** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `nslookup docker.com` | Thành phần `nslookup docker.com` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
