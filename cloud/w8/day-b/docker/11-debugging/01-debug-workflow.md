# Workflow debug Docker

Chapter: Debugging và Troubleshooting

## Mục tiêu

- Hiểu bản chất của `symptom`, `hypothesis`, `evidence`, `fix`.
...
Trong Docker, chủ đề **Workflow debug Docker** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Workflow debug Docker"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Workflow debug Docker** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker events` | Thành phần `docker events` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--since 10m` | Thành phần `--since 10m` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
