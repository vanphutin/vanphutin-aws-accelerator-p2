# OOMKilled và CrashLoop

Chapter: Debugging và Troubleshooting

## Mục tiêu

- Hiểu bản chất của `OOMKilled`, `exit code`, `restart loop`.
...
Trong Docker, chủ đề **OOMKilled và CrashLoop** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["OOMKilled và CrashLoop"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **OOMKilled và CrashLoop** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker inspect` | Thành phần `docker inspect` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `app` | Thành phần `app` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--format '{{.State.OOMKilled}} {{.State.ExitCode}}'` | Thành phần `--format '{{.State.OOMKilled}} {{.State.ExitCode}}'` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
