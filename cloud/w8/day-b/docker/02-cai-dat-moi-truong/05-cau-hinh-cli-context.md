# Cấu hình Docker CLI và context

Chapter: Cài đặt môi trường

## Mục tiêu

- Hiểu bản chất của `context`, `remote daemon`, `DOCKER_HOST`, `TLS`.
...
Trong Docker, chủ đề **Cấu hình Docker CLI và context** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Cấu hình Docker CLI và context"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Cấu hình Docker CLI và context** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker context use` | Thành phần `docker context use` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `default` | Thành phần `default` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
