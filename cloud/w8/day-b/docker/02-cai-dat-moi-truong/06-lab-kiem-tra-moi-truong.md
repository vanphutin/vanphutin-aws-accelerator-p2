# Lab kiểm tra môi trường

Chapter: Cài đặt môi trường

## Mục tiêu

- Hiểu bản chất của `smoke test`, `volume mount`, `network test`.
...
Trong Docker, chủ đề **Lab kiểm tra môi trường** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Lab kiểm tra môi trường"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Lab kiểm tra môi trường** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `${PWD}:/work` | Thành phần `${PWD}:/work` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `alpine` | Thành phần `alpine` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `ls -la /work` | Thành phần `ls -la /work` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
