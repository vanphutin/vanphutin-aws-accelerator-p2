# Cài đặt Docker Engine trên Linux

Chapter: Cài đặt môi trường

## Mục tiêu

- Hiểu bản chất của `apt/yum`, `service`, `daemon`, `group docker`.
...
Trong Docker, chủ đề **Cài đặt Docker Engine trên Linux** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Cài đặt Docker Engine trên Linux"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Cài đặt Docker Engine trên Linux** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `sudo` | Thành phần `sudo` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `systemctl status` | Thành phần `systemctl status` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `docker` | Thành phần `docker` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
