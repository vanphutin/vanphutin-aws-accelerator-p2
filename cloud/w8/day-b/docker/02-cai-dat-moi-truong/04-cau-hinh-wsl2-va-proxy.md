# Cấu hình WSL2 và proxy

Chapter: Cài đặt môi trường

## Mục tiêu

- Hiểu bản chất của `WSL2`, `proxy`, `DNS`, `corporate network`.
...
Trong Docker, chủ đề **Cấu hình WSL2 và proxy** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Cấu hình WSL2 và proxy"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Cấu hình WSL2 và proxy** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `ping -c 3 google.com` | Thành phần `ping -c 3 google.com` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
