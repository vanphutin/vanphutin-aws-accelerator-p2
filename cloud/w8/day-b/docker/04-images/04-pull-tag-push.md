...
- Hiểu bản chất của `registry`, `tagging`, `push`, `authentication`.
...
Trong Docker, chủ đề **Pull, Tag và Push** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Pull, Tag và Push"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Pull, Tag và Push** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker tag nginx:alpine myrepo/nginx:demo
```
...
| `myrepo/nginx:demo` | Thành phần `myrepo/nginx:demo` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
