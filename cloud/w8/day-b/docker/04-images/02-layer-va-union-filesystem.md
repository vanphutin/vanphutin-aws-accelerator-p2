# Layer và Union File System

Chapter: Docker Images

## Mục tiêu

- Hiểu bản chất của `layer`, `overlay2`, `copy-on-write`, `history`.
...
Trong Docker, chủ đề **Layer và Union File System** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Layer và Union File System"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Layer và Union File System** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker history` | Thành phần `docker history` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
