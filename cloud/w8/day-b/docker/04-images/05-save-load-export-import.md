...
- Hiểu bản chất của `save`, `load`, `export`, `import`, `air-gapped`.
...
Trong Docker, chủ đề **Save, Load, Export, Import** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Save, Load, Export, Import"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Save, Load, Export, Import** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker save` | Thành phần `docker save` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `nginx:alpine` | Image Nginx dùng Alpine Linux, nhỏ hơn bản đầy đủ. |
| `-o nginx.tar` | Thành phần `-o nginx.tar` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
