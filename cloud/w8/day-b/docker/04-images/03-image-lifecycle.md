...
- Hiểu bản chất của `pull`, `build`, `tag`, `push`, `remove`.
...
Trong Docker, chủ đề **Image Lifecycle** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Image Lifecycle"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Image Lifecycle** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker images` | Thành phần `docker images` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
