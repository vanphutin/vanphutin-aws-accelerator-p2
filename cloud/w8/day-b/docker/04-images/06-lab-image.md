...
- Hiểu bản chất của `audit`, `cleanup`, `offline transfer`.
...
Trong Docker, chủ đề **Lab quản lý image** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Lab quản lý image"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Lab quản lý image** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker image prune` | Thành phần `docker image prune` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
