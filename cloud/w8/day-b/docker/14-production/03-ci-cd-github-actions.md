...
- Hiểu bản chất của `pipeline`, `buildx`, `push`, `cache`.
...
Trong Docker, chủ đề **CI/CD với GitHub Actions** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["CI/CD với GitHub Actions"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **CI/CD với GitHub Actions** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker buildx build --push -t ghcr.io/org/app:sha .
```
...
| `-t ghcr.io/org/app:sha` | Thành phần `-t ghcr.io/org/app:sha` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
