...
- Hiểu bản chất của `cgroups`, `CPU shares`, `memory limit`, `OOM`.
...
Trong Docker, chủ đề **Resource Limits, cgroups** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Resource Limits, cgroups"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Resource Limits, cgroups** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker run --memory 256m --cpus 0.5 nginx:alpine
```
...
| `--memory 256m` | Giới hạn bộ nhớ container ở mức 256 MB. |
| `--cpus 0.5` | Giới hạn container dùng tối đa nửa CPU. |
