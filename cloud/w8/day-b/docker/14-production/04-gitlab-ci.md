# GitLab CI cho Docker

Chapter: Docker Production

## Mục tiêu

- Hiểu bản chất của `runner`, `registry`, `variables`.
...
Trong Docker, chủ đề **GitLab CI cho Docker** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["GitLab CI cho Docker"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **GitLab CI cho Docker** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `-t registry.example.com/app:$CI_COMMIT_SHA` | Thành phần `-t registry.example.com/app:$CI_COMMIT_SHA` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
