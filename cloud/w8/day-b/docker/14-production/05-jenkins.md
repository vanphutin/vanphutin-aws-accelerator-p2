...
- Hiểu bản chất của `agent`, `credentials`, `stages`.
...
Trong Docker, chủ đề **Jenkins Pipeline** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Jenkins Pipeline"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Jenkins Pipeline** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `-t app:${BUILD_NUMBER}` | Thành phần `-t app:${BUILD_NUMBER}` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
