...
Chapter: Docker cơ bản

## Mục tiêu

- Hiểu bản chất của `hands-on`, `inspect`, `logs`, `remove`.
...
Trong Docker, chủ đề **Lab Docker cơ bản** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Lab Docker cơ bản"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Lab Docker cơ bản** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker run --name lab-web -d -p 8081:80 nginx:alpine
```
...
| `--name lab-web` | Thành phần `--name lab-web` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `-d` | Chạy ở chế độ nền, trả lại terminal ngay sau khi container/service khởi động. |
| `-p` | Publish cổng từ container ra host. |
| `8081:80` | Thành phần `8081:80` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `nginx:alpine` | Image Nginx dùng Alpine Linux, nhỏ hơn bản đầy đủ. |
