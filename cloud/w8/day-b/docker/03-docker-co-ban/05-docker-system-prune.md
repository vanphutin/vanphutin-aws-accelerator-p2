# Dọn dẹp Docker an toàn

Chapter: Docker cơ bản

## Mục tiêu

- Hiểu bản chất của `dangling`, `unused`, `cache`, `risk`.
...
Trong Docker, chủ đề **Dọn dẹp Docker an toàn** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Dọn dẹp Docker an toàn"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Dọn dẹp Docker an toàn** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker system prune` | Thành phần `docker system prune` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `-f` | Thành phần `-f` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
