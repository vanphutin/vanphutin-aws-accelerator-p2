# Quản lý đối tượng Docker

Chapter: Docker cơ bản

## Mục tiêu

- Hiểu bản chất của `list`, `filter`, `format`, `cleanup`.
...
Trong Docker, chủ đề **Quản lý đối tượng Docker** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Quản lý đối tượng Docker"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Quản lý đối tượng Docker** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker ps` | Thành phần `docker ps` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `-a` | Thành phần `-a` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
