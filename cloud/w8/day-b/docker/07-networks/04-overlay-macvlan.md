...
Chapter: Docker Networks

## Mục tiêu

- Hiểu bản chất của `multi-host`, `swarm`, `macvlan`, `L2`.
...
Trong Docker, chủ đề **Overlay và Macvlan** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Overlay và Macvlan"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Overlay và Macvlan** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker network create` | Thành phần `docker network create` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `-d overlay` | Thành phần `-d overlay` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `app-overlay` | Thành phần `app-overlay` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
