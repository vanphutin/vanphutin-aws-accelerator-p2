...
- Hiểu bản chất của `connectivity`, `curl`, `troubleshoot`.
...
Trong Docker, chủ đề **Lab networking** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Lab networking"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Lab networking** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `--network app-net` | Thành phần `--network app-net` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
