# Host và None Network

Chapter: Docker Networks

## Mục tiêu

- Hiểu bản chất của `host`, `none`, `security tradeoff`.
...
Trong Docker, chủ đề **Host và None Network** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Host và None Network"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Host và None Network** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `--network host` | Thành phần `--network host` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
