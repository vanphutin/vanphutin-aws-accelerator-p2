...
- Hiểu bản chất của `embedded DNS`, `service name`, `aliases`.
...
Trong Docker, chủ đề **DNS và Service Discovery** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["DNS và Service Discovery"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **DNS và Service Discovery** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `nslookup web` | Thành phần `nslookup web` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
