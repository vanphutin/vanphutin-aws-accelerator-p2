# Thực hành debug phỏng vấn

Chapter: Docker Interview

## Mục tiêu

- Hiểu bản chất của `debug`, `observability`, `OOM`.
...
Trong Docker, chủ đề **Thực hành debug phỏng vấn** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Thực hành debug phỏng vấn"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Thực hành debug phỏng vấn** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker stats` | Thành phần `docker stats` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--no-stream` | Thành phần `--no-stream` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
