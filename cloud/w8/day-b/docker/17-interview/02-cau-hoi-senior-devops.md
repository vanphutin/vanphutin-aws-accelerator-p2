# Câu hỏi Senior DevOps

Chapter: Docker Interview

## Mục tiêu

- Hiểu bản chất của `production`, `security`, `incident`, `tradeoff`.
...
Trong Docker, chủ đề **Câu hỏi Senior DevOps** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Câu hỏi Senior DevOps"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Câu hỏi Senior DevOps** trên máy local trước khi đưa vào pipeline hoặc server lab.
