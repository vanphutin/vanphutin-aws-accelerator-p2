...
Chapter: Giới thiệu Docker

## Mục tiêu

- Hiểu bản chất của `Kernel sharing`, `Hypervisor`, `Guest OS`, `Startup time`.
...
Trong Docker, chủ đề **Virtual Machine vs Container** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Virtual Machine vs Container"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Virtual Machine vs Container** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker run -it --name demo-ubuntu ubuntu:22.04 bash
```
...
| `-it` | Thành phần `-it` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--name demo-ubuntu` | Thành phần `--name demo-ubuntu` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `ubuntu:22.04` | Thành phần `ubuntu:22.04` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `bash` | Thành phần `bash` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
