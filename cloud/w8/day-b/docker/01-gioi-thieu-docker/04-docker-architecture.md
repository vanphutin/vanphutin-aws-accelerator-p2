...
- Hiểu bản chất của `Docker CLI`, `Docker daemon`, `Docker Engine API`, `containerd`, `runc`.
...
Trong Docker, chủ đề **Docker Architecture** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Docker Architecture"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Docker Architecture** trên máy local trước khi đưa vào pipeline hoặc server lab.
