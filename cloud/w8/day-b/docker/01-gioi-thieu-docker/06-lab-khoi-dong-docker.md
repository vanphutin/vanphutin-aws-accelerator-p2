# Lab khởi động Docker

Chapter: Giới thiệu Docker

## Mục tiêu

- Hiểu bản chất của `first container`, `port mapping`, `logs`, `cleanup`.
...
Trong Docker, chủ đề **Lab khởi động Docker** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Lab khởi động Docker"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Lab khởi động Docker** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `8080:80` | Ánh xạ cổng host 8080 vào cổng container 80. |
| `--name web` | Đặt tên container là `web` để dễ quản lý. |
| `nginx` | Image Nginx mặc định từ Docker Hub. |
