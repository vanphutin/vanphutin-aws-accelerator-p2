# OCI và tiêu chuẩn container

Chapter: Giới thiệu Docker

## Mục tiêu

- Hiểu bản chất của `OCI Image Spec`, `OCI Runtime Spec`, `runc`, `portability`.
...
Trong Docker, chủ đề **OCI và tiêu chuẩn container** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["OCI và tiêu chuẩn container"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **OCI và tiêu chuẩn container** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker image inspect` | Thành phần `docker image inspect` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
