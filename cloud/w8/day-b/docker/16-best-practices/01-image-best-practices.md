...
- Hiểu bản chất của `pin tag`, `small image`, `scan`.
...
Trong Docker, chủ đề **Best Practices cho image** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Best Practices cho image"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Best Practices cho image** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker build --pull --no-cache -t app:clean .
```
...
| `--pull` | Luôn kiểm tra và kéo base image mới hơn nếu có. |
| `--no-cache` | Không dùng cache cũ khi build. |
| `-t app:clean` | Thành phần `-t app:clean` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
