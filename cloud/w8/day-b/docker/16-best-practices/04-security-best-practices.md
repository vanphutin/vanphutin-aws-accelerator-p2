...
- Hiểu bản chất của `least privilege`, `capabilities`, `secrets`.
...
Trong Docker, chủ đề **Best Practices Security** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Best Practices Security"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Best Practices Security** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker run --rm --user 10001:10001 nginx:alpine
```
...
| `--user 10001:10001` | Thành phần `--user 10001:10001` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
