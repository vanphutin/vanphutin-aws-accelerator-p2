...
- Hiểu bản chất của `cache`, `resources`, `startup`, `I/O`.
...
Trong Docker, chủ đề **Best Practices Performance** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Best Practices Performance"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Best Practices Performance** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
|---|---|
| `docker buildx du` | Thành phần `docker buildx du` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
