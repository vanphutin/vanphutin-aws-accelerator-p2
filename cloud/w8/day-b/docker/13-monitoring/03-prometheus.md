...
- Hiểu bản chất của `scrape`, `PromQL`, `targets`.
...
Trong Docker, chủ đề **Prometheus** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Prometheus"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Prometheus** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `9090:9090` | Thành phần `9090:9090` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `prom/prometheus` | Thành phần `prom/prometheus` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
