...
Chapter: Monitoring Docker

## Mục tiêu

- Hiểu bản chất của `container metrics`, `cgroups`, `scrape`.
...
Trong Docker, chủ đề **cAdvisor** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["cAdvisor"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **cAdvisor** trên máy local trước khi đưa vào pipeline hoặc server lab.

```bash
docker run -d -p 8080:8080 --name cadvisor gcr.io/cadvisor/cadvisor:latest
```
...
| `-p` | Publish cổng từ container ra host. |
| `8080:8080` | Thành phần `8080:8080` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `--name cadvisor` | Thành phần `--name cadvisor` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `gcr.io/cadvisor/cadvisor:latest` | Thành phần `gcr.io/cadvisor/cadvisor:latest` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
