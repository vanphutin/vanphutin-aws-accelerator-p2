...
- Hiểu bản chất của `dashboard`, `datasource`, `alert`.
...
Trong Docker, chủ đề **Grafana** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Grafana"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Grafana** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `3000:3000` | Thành phần `3000:3000` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `grafana/grafana` | Thành phần `grafana/grafana` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
