...
- Hiểu bản chất của `log aggregation`, `labels`, `retention`.
...
Trong Docker, chủ đề **Loki Logs** cần được hiểu theo hướng thực chiến: không chỉ nhớ định nghĩa, mà phải biết đối tượng nào được tạo ra, dữ liệu nằm ở đâu, quyền hạn ra sao và khi lỗi thì kiểm tra từ lớp nào trước.
...
Engine --> Topic["Loki Logs"]
Topic --> Result["Ứng dụng chạy nhất quán"]
```

## Ví dụ

Tình huống: bạn cần kiểm chứng nhanh chủ đề **Loki Logs** trên máy local trước khi đưa vào pipeline hoặc server lab.
...
| `3100:3100` | Thành phần `3100:3100` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
| `grafana/loki` | Thành phần `grafana/loki` của command; đọc theo ngữ cảnh bài học để hiểu vai trò cụ thể. |
